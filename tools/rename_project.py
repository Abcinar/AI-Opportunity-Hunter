...
# ---------------------------------------------------------------------------
# Protection layer (highest priority)
# ---------------------------------------------------------------------------


def is_protected_path(path: Path, *, root: Optional[Path] = None) -> bool:
    """Return True if the path must never be scanned, read, modified, renamed,
    backed up or restored over.

    This is the highest-priority safety gate. It protects:
    - The running script itself
    - All configuration files used by the tool
    - Generated reports
    - Log files
    - Backup directories and their contents
    - Well-known VCS / IDE / virtual-env / build directories
    - Symbolic links (always treated as protected)

    Args:
        path: Absolute or relative path to inspect.
        root: Optional repository root. When supplied, relative path
              calculations are performed against it for more precise matching.

    Returns:
        True when the path is protected and must be left untouched.
    """
    try:
    ...
    # Symbolic links are never touched
    if path.is_symlink() or resolved.is_symlink():
        return True

    name = resolved.name

    # Exact protected file names
    if name in PROTECTED_FILES:
        return True

    # Any path component that is a protected directory name
    for part in resolved.parts:
        if part in PROTECTED_DIRS:
            return True

    # Extra safety: if root is known, also check the relative path string
    if root is not None:
        try:
...
class RenameConfig:
    ...
    ignore_dirs: List[str] = field(
        default_factory=lambda: list(PROTECTED_DIRS)
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "RenameConfig":
        ...
        return cls(
            ...
            ignore_dirs=data.get("ignore_dirs", list(PROTECTED_DIRS)),
...
class RenameReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "skipped_protected_count": len(self.skipped_protected),
            "skipped_protected": self.skipped_protected[:100],
...
class PathFilter:
    ...
    def __init__(
        ...
        """
        self._ignore_dirs: Set[str] = set(ignore_dirs) | set(PROTECTED_DIRS)
...
class RepositoryScanner:
    ...
    def iter_files(self) -> Iterator[Path]:
        """Yield every non-protected, non-ignored file under the root.

        Yields:
            Absolute Path objects for candidate files.
        """
        for dirpath, dirnames, filenames in os.walk(
            self.root, topdown=True, followlinks=False
        ):
            # Prune protected / ignored directories
            dirnames[:] = [
                d
                for d in dirnames
                if not self.path_filter.is_ignored_dir(d)
                and not is_protected_path(Path(dirpath) / d, root=self.root)
            ]
            current = Path(dirpath)
            if is_protected_path(current, root=self.root):
                continue
            for filename in filenames:
                file_path = current / filename
                if is_protected_path(file_path, root=self.root):
                    continue
                yield file_path

    def iter_directories(self) -> List[Path]:
        """Collect all non-protected directories under the root (deepest first).
        ...
        for dirpath, dirnames, _ in os.walk(
            ...
            current = Path(dirpath)
            if current != self.root and not is_protected_path(current, root=self.root):
                dirs.append(current)
...
class BackupManager:
    ...
    def backup_file(self, source: Path, root: Path) -> Path:
        ...
        Protected paths are never backed up.
        ...
        Raises:
            RuntimeError: If no session has been created or path is protected.
            OSError: On filesystem errors.
        """
        if is_protected_path(source, root=root):
            raise RuntimeError(f"Refusing to back up protected path: {source}")
    ...
    def undo_latest(self, root: Path) -> Tuple[int, List[str]]:
        ...
        Protected destination paths are never overwritten.
        ...
        for backup_file in latest.rglob("*"):
            ...
            target = root / rel
            if is_protected_path(target, root=root):
                errors.append(f"{rel}: protected path – skipped")
                continue
            try:
...
class RenameService:
    ...
    def _process_file(
        ...
        # Highest-priority protection gate
        if is_protected_path(file_path, root=self.root):
            report.skipped_protected.append(rel)
            self.logger.debug("Skipping protected path: %s", rel)
            return FileResult(
                path=rel,
                replacements=0,
                modified=False,
        ...
        # Symlink safety (also covered by is_protected_path, kept for clarity)
    ...
    def _rename_paths(self, report: RenameReport, *, dry_run: bool) -> None:
        ...
        Protected paths are never renamed.
        ...
        for dir_path in directories:
            if is_protected_path(dir_path, root=self.root):
                continue
    ...
    def _try_rename_path(
        ...
        *,
        ...
        """
        if is_protected_path(path, root=self.root):
            return
        ...
        # Also protect the destination name if it would collide with a protected name
        if new_name in PROTECTED_FILES or new_name in PROTECTED_DIRS:
            self.logger.warning(
                "Refusing to rename %s to protected name %s", rel, new_name
            )
            return

        new_path = path.with_name(new_name)
        if is_protected_path(new_path, root=self.root):
            self.logger.warning(
                "Refusing to rename %s – destination is protected", rel
            )
            return
        ...
        if dry_run:
            self.logger.info("[DRY-RUN] Would rename %s → %s", rel, new_rel)
            self._print(
    ...
    def _print_summary(self, report: RenameReport) -> None:
        ...
        self._print(
            f"  Skipped protected paths  : {len(report.skipped_protected)}",
            Color.YELLOW,
        )
        self._print(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  rename_project.py, rename_config.*, report.*, logs/, backup/, .git/ …\n"
        ),
    )
    parser.add_argument(
        "--version",
